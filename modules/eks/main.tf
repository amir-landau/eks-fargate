...
resource "helm_release" "aws-load-balancer-controller" {
  ...
  depends_on = [module.fargate_profile["kube_system"]]
...
locals {
  kubeconfig = yamlencode({
    ...
    clusters = [{
      name = aws_eks_cluster.this[0].name
      cluster = {
        certificate-authority-data = aws_eks_cluster.this[0].certificate_authority[0].data
    ...
    contexts = [{
      name = "terraform"
      context = {
        cluster = aws_eks_cluster.this[0].name
        user    = "terraform"
      }
    }]
    users = [{
...
resource "null_resource" "modify_kube_dns" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      echo "Setting implicit dependency on ${module.fargate_profile["kube_system"].fargate_profile_pod_execution_role_arn}"
